...
docker exec formmicrologger-ftpd-server bash -c 'echo -e "Passw0rd\nPassw0rd" > /tmp/bobp.txt'
docker exec formmicrologger-ftpd-server bash -c 'pure-pw useradd bob -u ftpuser -d /home/ftpusers/bob < /tmp/bobp.txt'
docker exec formmicrologger-ftpd-server pure-pw mkdb
